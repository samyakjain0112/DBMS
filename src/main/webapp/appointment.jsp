...
<div class="form1">				
<form action="/customer/makeappointment" >
<h3>Book an Appointment</h3>
<br>
<label for ="time">Time of Appointment in hrs(10-20)</label>
<input type="number" id="time" name="time"  min="10" max="20" required="required"><br>

<label for ="dob">Date of Appointment</label>
<input type="date" id= "date" name= "date" required="required"><br>
<select id="treatmentId" name="treatmentId" required="required">
<option value="">Choose the facility</option>
	    <c:forEach var="facilities" items="${getallfacilities}">
	       <option value="${facilities.getTreatmentId()}">${facilities.getTreatmentId()} ${facilities.getName()}  ${facilities.getStaffId()}</option>   
	   </c:forEach>
	   </select>	  
